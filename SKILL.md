...
현재 대화 컨텍스트(이 대화 전체)를 보고 정리한다.

### 3.0 중복 방지 — 대화 내 이전 `/work_done` 호출 감지 (필수)

같은 Claude Code 세션을 여러 날·여러 작업 episode에 걸쳐 유지하는 사용자의 경우, 한 대화 안에서 `/work_done`이 2번 이상 호출될 수 있다. 이때 두 번째 이후 호출이 첫 호출과 같은 옛 작업을 다시 적으면 세션 파일이 중복된다.

**식별 방법** — 본 호출 직전까지의 대화 컨텍스트에서 다음 흔적이 있는지 확인:
- 사용자의 이전 `/work_done` 명령 메시지 (command-message 블록 등)
- 이전에 `_sessions/*.md`에 쓴 Write 도구 호출
- 이전 "세션 파일 저장: …" 보고 블록

**이전 호출이 있으면**:
- 그 시점을 **cutoff**로 인식
- 본 호출의 세션 파일에는 **cutoff 이후 새로 일어난 작업만** 4섹션으로 정리
- "왜" 섹션에서 cutoff 이전 작업을 **맥락 참조**하는 것은 허용 (예: "어제 [b8d4]에서 B 완성 후 오늘 Y로 확장 진입")

**중요 — 컨텍스트 이해는 그대로 유지**:
- cutoff 이전 메시지를 무시하라는 게 아님
- LLM은 대화 전체를 계속 이해·활용 (사용자 질문 답변, 옛 결정 이어받기, 참조 등)
- 단지 **출력하는 세션 파일에 옛 작업을 다시 적지 않을 뿐**

**첫 호출** (이 대화 내 이전 `/work_done` 흔적 없음) → 처음부터 전체 정리 (기존 동작).

### 3.1 추출 대상 (4섹션)

- **한 일 (What):** 실제로 수행한 작업·변경·산출물. 동사로 시작하는 불릿. 파일명·함수명 등 구체적인 식별자를 포함. **cutoff 이후 작업만.**
- **왜 (Why):** 그 작업이 필요했던 동기, 해결하려던 문제, 사용자가 명시한 맥락. **cutoff 이전 작업을 맥락 참조 가능.**
- **의사결정·트레이드오프:** 두 개 이상의 대안 중 무엇을 골랐고 왜 그랬는지. 사용자와 의논한 선택을 우선 기록. **cutoff 이후 결정만.**
- **다음 (Next):** 미완료 작업, 후속으로 할 일, 사용자가 미루기로 한 것. **cutoff 이후 발생한 미완료만** (이미 옛 세션 파일에 적힌 "다음"은 다시 적지 않음).

### 3.2 추가 추론

- **topics:** 작업의 주제 태그 (예: `SBR`, `python-script`, `refactor`). 2-5개. **cutoff 이후 작업 기준.**
