...
3단계에서 추론한 모든 태그(topics + projects)를 `<work-log>/_index/tags.md` 사전과 대조해 일관성을 맞춘다. 이게 두 달 뒤 회수 정확도를 결정한다.

절차:
```
1. <work-log>/_index/tags.md를 Read.
2. 사전을 파싱: {표준명: [동의어 리스트], category: "공정"|"작업유형"|"프로젝트"|"도구"|"기타"}

3. 추론된 각 태그에 대해:
   a. 사전의 표준명·동의어 전체와 정확 매칭 시도
      → 매칭되면 표준명으로 자동 변환 (사용자에게 안 물어봄)
   b. 정확 매칭 실패 시 퍼지 매칭 (편집 거리, 부분 문자열):
      유사도 높은 후보 발견 → 사용자에게 "<신규> ↔ <기존 표준명> 통합?" 물어봄
      - Y: 신규를 기존 표준명으로 변환 + 동의어로 사전에 추가
      - N: 진짜 신규로 처리 (다음 단계로)
   c. 매칭 실패 시 신규 태그:
      "새 태그 '<신규>' 발견. 카테고리는? [공정/작업유형/프로젝트/도구/기타]"
      사용자 선택 후 사전에 추가

4. 정규화된 최종 태그 집합으로 4단계 진행.
```

사전 갱신 로직:
- 신규 표준명 추가: 해당 카테고리 섹션 끝에 `- <표준명>` 한 줄 추가
- 신규 동의어 추가: 기존 표준명 줄의 괄호 안에 동의어 추가 `- SBR (sbr, ..., 신규동의어)`
- 카테고리·표준명 변경 등 복잡한 작업은 사용자가 직접 `tags.md`를 편집 (다음 호출에 자동 반영됨)

사전이 비정상(파일 손상, 파싱 실패)이면 정규화 단계를 건너뛰고 사용자에게 알림. 작성은 막지 않음.
...
- `references/format.md` — 일일 파일 / 주제 인덱스 / 태그 사전의 상세 템플릿과 예시
- `references/index-rebuild.md` — 주제 인덱스 재생성의 단계별 의사 코드
- `references/tags-seed.md` — 첫 실행 시 `_index/tags.md`에 복사할 초기 태그 사전 (사용자 도메인 기반)
